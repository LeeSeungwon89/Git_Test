# 깃 연습장
깃을 연습하는 저장소입니다. 불필요한 커밋은 삭제합니다.

Git을 공부하기에 좋은 서적은 **팀 개발을 위한 Git, GitHub 시작하기(한빛미디어)** 입니다.
설명이 매우 쉽고 구체적입니다. 차근차근 읽고 실습하다 보면 현업에서 깃을 사용하기에 부족하지 않은(충분한) 수준에 도달할 것입니다.

# 깃 명령어

자주 쓰이는 CLI 명령어를 목록화합니다. `[옵션인자]`는 생략 가능, `<필수인자>`는 생략 불가능입니다. 지속적으로 업데이트 중입니다.

명령어 사용에 숙달해지면 작업 속도를 높이거나 세세하게 제어하기 위해 소스트리보다는 CLI를 사용하는 편이 낫습니다. 물론 소스트리만 사용해도 충분한 경우도 많습니다.

## 도움말 기능 사용하기

- 명령어의 도움말을 확인합니다.
```
git help <명령어>
# ex) `git help commit`: 커밋 명령어에 대한 도움말을 확인합니다.
```

## 폴더 확인, 생성, 이동하기

- 현재 폴더 위치를 확인합니다.
```
pwd
```

- 현재 폴더 내 파일 목록을 확인합니다.
```
# `-a` 옵션을 사용하면 숨김 파일까지 확인합니다.
ls [-a]
```

- 파일 내용을 확인합니다.
```
cat [파일명]
```

- 홈 폴더로 이동합니다.
```
cd
```

- 특정 디렉토리로 이동합니다.
```
cd <폴더명>
```

- 현재 폴더의 상위 폴더로 이동합니다.
```
cd ../
```

- 현재 폴더에 새 폴더를 만듭니다.
```
mkdir <폴더명>
```

- 화면에 문장을 표시합니다.
```
echo "문장"
```

- 파일을 생성합니다.
```
echo "내용" > 파일명 
```

- Git 저장소(워킹트리) 상태를 확인합니다.
```
git status
```

- Git 저장소(워킹트리) 상태를 요약하여 확인합니다.
```
git status -s
```

- 폴더에서 버전 관리를 시작합니다. `.git`이라는 숨김 폴더(로컬저장소)가 생성됩니다.
```
git init
```

## 옵션 설정하기

- 전역 옵션 내용을 확인합니다. 전역 옵션은 개인 PC에서 많이 사용합니다.
```
git config --global <옵션>
```

- 전역옵션 값을 새로 설정합니다.
```
git config --global <옵션> <새로운 값>
```

- 전역옵션을 삭제합니다.
```
git config --global --unset <옵션>
```

- 지역옵션 내용을 확인합니다. 지역옵션은 공용 PC를 사용하거나 Git을 잠시 사용할 때 설정합니다.
```
git config --local <옵션>
```

- 지역옵션 값을 새로 설정합니다.
```
git config --local <옵션> <새로운 값>
```

- 지역옵션 값을 삭제합니다.
```
git config --local --unset <옵션>
```

- 시스템옵션 내용을 확인합니다. 
```
git config --system <옵션>
```

- 시스템옵션 값을 새로 설정합니다.
```
git config --system <옵션> <새로운 값>
```

- 시스템옵션 값을 삭제합니다.
```
git config --system --unset <옵션> <값>
```

- 현 프로젝트의 모든 옵션을 확인합니다.
```
git config --list
```

- Git의 기본 에디터를 확인합니다. 기본 에디터를 설정할 때는 Git을 재설치하여 설정하는 편이 좋습니다.
```
git config core.editor
git config --global core.editor
git config --system core.editor
```

- 현재 사용 중인 유저명을 확인합니다.
```
git config --global user.name
```

- 현재 사용 중인 유저의 메일주소를 확인합니다.
```
git config --global user.email
```

- 깃에 계정을 연동합니다.
```
git config --global user.email "메일주소"
git config --global user.name "계정명"
```

## staging 및 commit하기

- 커밋에 추가할(스테이지에 추가할) 파일을 선택합니다.
```
git add 파일1 파일2 ...
```

- 커밋합니다.
```
git commit
```

- `add` 명령을 생략하고 바로 커밋합니다. 변경된 파일과 삭제된 파일을 자동으로 스테이징하고 커밋합니다. 단, `untracked` 파일은 커밋하지 않습니다.
```
git commit -a
```

- 상세설명을 추가하여 커밋합니다.
```
# `m`은 massage의 약자입니다. 
git commit -m "설명"
```

- 커밋 목록을 확인합니다.
```
git log
```

- 이전 커밋으로 돌아갑니다.
```
git checkout 커밋 코드 앞 부분 7자리
```

- 최신 커밋으로 돌아갑니다.
```
git checkout -
```

- 현재 프랜치의 커밋 이력을 확인합니다. `HEAD`와 관련된 커밋들을 자세하게 출력합니다.
```
git log
```

- 전체 커밋 중에 최신 n개의 커밋만 확인합니다.
```
git log -n
```

- 여러 옵션을 조합하여 커밋을 확인합니다. 소스트리로 확인하는 것을 권장합니다.
```
# `--oneline`: 커밋 메시지를 한 줄로 요약합니다. 커밋 해시와 제목만 확인합니다. 생략하면 자세히 표시합니다.
# `--graph`: 커밋 옆에 브랜치의 흐름을 그래프화합니다.
# `--decorate`: 브랜치와 태그 등 참조를 간결히 표시합니다.
# `--all`: all 옵션이 없으면 `HEAD`와 관계없는 옵션을 보이지 않습니다.
# 모든 옵션을 사용하면 전체 브랜치를 확인할 수 있습니다.
git log --oneline --graph --decorate --all

# 브랜치의 최신 커밋을 5개만 확인합니다.
git log --oneline -n5
```

- 커밋 메시지 작성 7가지 방법
1. 제목과 본문은 빈 줄로 나눕니다.
```
제목

본문
```
2. 제목
   - 제목이 영어일 경우 첫 글자는 대문자로 사용합니다.
   - 제목이 영어일 경우 동사원형(현재형)을 사용합니다.
   - 제목에 마침표를 사용하지 않습니다.
3. 본문
   - 72자 단위로 줄을 바꿉니다.
4. HOW보다 WHAT과 WHY를 명시합니다.

## commit 취소하기
- 특정 커밋을 취소하고 취소한 커밋을 생성합니다.
```
git revert <커밋 코드>
```

## commit 되돌리기
- 현재 브랜치를 특정 커밋으로 되돌립니다. 작업 폴더의 내용도 함께 변경됩니다.
```
# 커밋 체크섬 대신 `HEAD~` 또는 `HEAD^`을 사용할 수 있습니다.
# `HEAD~<숫자>`에서 `HEAD~`은 헤드의 부모 커밋, `HEAD~2`는 헤드의 할아버지 커밋을 의미합니다.
# 즉, `HEAD~<숫자>`는 숫자 n번째 위쪽 조상입니다.
# `git checkout HEAD~2`를 사용하면 `HEAD`만 옮겨집니다.
# `HEAD^`은 부모 커밋입니다. `HEAD^2`는 두 번째 부모를 의미합니다.
# 즉, 병합 커밋처럼 부모가 둘 이상인 커밋에서만 의미가 있습니다.
git reset --hard <이동할 커밋 체크섬>

# 예컨대 현재 브랜치를 한 단계만 되돌리려면 아래와 같습니다.
# 병합 커밋에서 시도한 경우 병합 커밋이 사라지고 부모 커밋으로 돌아갑니다.
git reset --hard HEAD~
```

## push, pull, fetch, merge, remote, clone하기 

- 로컬저장소(master)의 커밋을 원격저장소(origin)에 올립니다(push).
```
git push origin master
```

- 현재 브랜치에서 새로 생성한 커밋들을 원격 저장소에 올립니다. `-u` 옵션을 설정하면 브랜치의 업스트림을 등록하며 한번 등록한 후에는 `git push`만 입력해도 됩니다.
```
git push [-u] [원격저장소 별명] [브랜치명]
```

- 원격저장소의 변경사항을 워킹트리에 반영합니다.
```
git pull
```

- 원격저장소의 커밋을 로컬 저장소에 내려받습니다(pull).
```
git pull origin master
```

- 원격저장소의 브랜치와 커밋들을 로컬저장소와 동기화합니다. 옵션을 생략하면 모든 원격저장소에서 모든 브랜치를 동기화합니다.
```
git fetch [원격저장소 별명] [브랜치 명]
```

- 지정한 프랜치의 커밋들을 현재 브랜치 및 워킹트리에 반영합니다.
```
git merge 브랜치명
```

- 원격저장소에 연동합니다.
```
git remote add origin <원격저장소 주소>
```

- 원격저장소 목록을 확인합니다.
```
git remote -v
```

- 원격저장소의 커밋을 로컬저장소에 복제(clone)합니다.
```
# 원격저장소 주소 뒤에 ' .'을 붙이면 폴더 안에 있는 파일만 복사합니다.
# 예컨대 `https://github.com/LeeSeungwon89/Git_Test.git .`형태입니다.
git clone <원격저장소 주소> [새로운 폴더명]
```

## staging 취소하기

- 스테이지 영역에 위치한 파일을 스테이지에서 내립니다(언스테이징). 워킹트리 내용은 변경되지 않으며 옵션을 생략하면 모든 변경사항을 스테이지에서 내립니다.
```
git reset [파일명] ...
```

## 브랜치 확인, 체크아웃, 생성 및 삭제하기

- 로컬저장소의 브랜치 목록을 확인합니다.
```
# `-v` 옵션을 사용하면 마지막 커밋도 확인합니다.
# 브랜치 중 `*`가 붙어 있으면 `HEAD` 브랜치입니다.
git branch [-v]
```

- 새 브랜치를 생성합니다.
```
# 커밋 체크섬을 가리키는 브랜치에서 새 브랜치를 생성합니다.
# 커밋체크섬을 생략하면 `HEAD`로부터 브랜치를 생성합니다.
# 이미 존재하는 브랜치를 다른 커밋으로 옮기려면 `-f` 옵션을 사용합니다.
git branch [-f] <브랜치 이름> [커밋 체크섬]
```

- 원격저장소에 있는 브랜치를 확인합니다.
```
# `v` 옵션을 사용하면 커밋 요약을 확인합니다.
git branch -r[v]
```

- 특정 브랜치로 체크아웃합니다. 브랜치 이름 대신 커밋 체크섬을 쓸 수 있으나 브랜치 이름을 사용하는 편이 좋습니다.
```
git checkout <브랜치 이름>
```

- 특정 커밋에서 브랜치를 새로 생성하고 체크아웃도 수행합니다.
```
git checkout -b <브랜치 이름> <커밋 체크섬>
```

- 현재 브랜치와 대상 브랜치를 병합합니다. 대부분 새 병합 커밋이 생성됩니다.
```
git merge <대상 브랜치>
```

- 내 브랜치의 커밋들을 대상 브랜치에 재배치합니다. 히스토리를 깔끔하게 만듭니다. 단, 원격저장소에는 사용하지 않는 것이 좋습니다.
```
git rebase <대상 브랜치>
```

- 특정 브랜치를 삭제합니다. `HEAD` 브랜치나 병합되지 않은 브랜치는 삭제할 수 없습니다.
```
git branch -d <브랜치 이름>
```

- 브랜치를 강제로 삭제합니다.
```
git branch -D <브랜치 이름>
```

- 임시 브랜치를 생성하여 자유롭게 작업을 수행해보는 것은 좋은 팁입니다.
```
git branch <임시 브랜치 이름> <부모 브랜치 이름>
```

## 버전에 태깅하기
- 주석 있는 태그를 생성합니다.
```
# `-a`로 주석 있는 태그를 생성합니다. 브랜치 이름을 생략하면 `HEAD`에 태그를 생성합니다.
git tag -a -m <메시지> <태그 이름> [브랜치 또는 체크섬]
```

- 원격저장소에 태그를 업로드합니다.
```
git push <원격저장소 별명> <태그 이름>
```

## 버그 처리 시나리오
- (옵션) 오류가 없는 버전(주로 `Tag`가 있는 커밋)으로 롤백
- `[master]` 브랜치로부터 `[hotfix]` 브랜치 생성(`git checkout -b hotfix master`)
- 소스 코드 수정 및 테스트 완료
- `[master]` 브랜치로 병합(Fast-forward) 및 배포
- 개발 중인 브랜치에도 병합(충돌 발생 가능성 높음)

# 소스트리

소스트리 설치 방법은 [링크](https://www.youtube.com/watch?v=f2qHHLZr3ME&t=439s)를 참고하시기 바랍니다. 별도의 계정 가입 절차 없이 깃허브 계정만 바로 사용할 수 있습니다. 자세한 사용법은 본서를 참고하시기 바랍니다.
