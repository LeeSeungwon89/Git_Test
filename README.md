...
- Git 저장소(워킹트리) 상태 확인하기
```
git status
```

- Git 저장소(워킹트리) 상태를 요약하여 확인하기
```
git status -s
```
...
사실 소스트리는 GUI라서 굳이 메모가 필요할까? 사용법이 간단해서 음. 일단 고민해보도록 하겠습니다. 소스트리 설치 방법은 [링크](https://www.youtube.com/watch?v=f2qHHLZr3ME&t=439s)를 참고하시기 바랍니다. 별도의 계정 가입 절차 없이 깃허브 계정만으로 바로 사용할 수 있습니다.
